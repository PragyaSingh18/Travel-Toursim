...
public class ViewPackage extends JFrame implements ActionListener {
    public ViewPackage(String username){
       ...
       getContentPane().setBackground(Color.white); //to change the color of frame
       setLayout(null);
       ...
       try{
        Conn c=new Conn(); // becoz we have to take the data from database
        ResultSet rs=c.s.executeQuery("select * from bookPackage where username='"+username+"'");
        while(rs.next()){
            l11.setText(rs.getString(1)); // we can give column number also as well as column name.
